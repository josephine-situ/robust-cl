...
Usage:
    ...
    # re-derive rho* under different criteria, no solving:
...
def run_sweep(config, args):
    ...
    scores_path = os.path.join(OUT_DIR, f"{problem}_rho_sweep.csv")
    ...
    tidy = os.path.join(OUT_DIR, f"{problem}_rho_curve.csv")
    ...
    if args.ablate:
        ...
        if ab:
            adf = pd.DataFrame(ab)
            apath = os.path.join(OUT_DIR, f"{problem}_ablations.csv")
...
def _rho_star_from_csv(problem, target, min_solved, sense="min",
                       exclude_capped=False, out_suffix=""):
    ...
    """
    path = os.path.join(OUT_DIR, f"{problem}_rho_curve.csv")
    if not os.path.exists(path):
        raise SystemExit(f"no {path}; run the sweep first")
    df = pd.read_csv(path)
    df = df[df.get("phase", "rho_sweep") == "rho_sweep"]
    if exclude_capped:
        df = df[df["n_capped"] == 0]
    return _rho_star(df, problem, target, sense, min_solved, out_suffix=out_suffix)
...
def main():
    ...
    p.add_argument("--out-suffix", default="",
                   help="suffix for the rho_star output, so several criteria can "
                        "coexist, e.g. --out-suffix _t080")
